import React from 'react';
import Layout from "../../../components/layout-black"
import { StaticImage } from "gatsby-plugin-image"
import Fancybox from "../../../components/fancybox.js";
import BreadcrumbsInterier from "../../../components/breadcrumbs-interier.js";
import { AnchorLink } from "gatsby-plugin-anchor-links"

const akvamarin = () => {
  return (
    <Layout>
      <Fancybox>
        <div class="wrapper">
          <div class="project">
            <BreadcrumbsInterier />
            <h1 class="project_title">Дизайн интерьера квартиры в&nbsp;ЖК&nbsp;&laquo;Аквамарин&raquo;, 140м<sup>2</sup></h1>
          </div>
          <div class="project_main_photo">
            <a
              href="https://www.i-project.by/portfolio_newsite/akvamarin/03_akvamarin.webp"
              data-fancybox="gallery"
            >
              <StaticImage src="../../../images/portfolio/akvamarin/03_akvamarin.jpg" alt="" />
            </a>
          </div>
        </div>
        <div class="content_body">
          <div class="content_body_absolute">
            <div class="left_sidebar">
              <ul>
                <li><AnchorLink to="/portfolio/interier/akvamarin/#anchor_1" title="Описание проекта" /></li>
                <li><AnchorLink to="/portfolio/interier/akvamarin/#anchor_2" title="Планировка квартиры" /></li>
                <li><AnchorLink to="/portfolio/interier/akvamarin/#anchor_3" title="Гостиная" /></li>
                <li><AnchorLink to="/portfolio/interier/akvamarin/#anchor_4" title="Кухня" /></li>
                <li><AnchorLink to="/portfolio/interier/akvamarin/#anchor_5" title="Прихожая" /></li>
                <li><AnchorLink to="/portfolio/interier/akvamarin/#anchor_6" title="Спальня" /></li>
                <li><AnchorLink to="/portfolio/interier/akvamarin/#anchor_7" title="Детские комнаты" /></li>
              </ul>
            </div>
          </div>
          <div class="wrapper">
            <div class="right_content">
              <div class="project_info">
                <p class="project_info_p">Год: <span>2015</span></p>
                <p class="project_info_p">Локация: <span>ЖК&nbsp;&laquo;Аквамарин&raquo;, Минск</span></p>
                <p class="project_info_p">Архитектор: <span>Александр Игнатьев</span></p>
              </div>
              <div class="content_task_block" id="anchor_1">
                <a
                  href="https://www.i-project.by/portfolio_newsite/akvamarin/07_akvamarin.webp"
                  data-fancybox="gallery"
                >
                  <StaticImage src="../../../images/portfolio/akvamarin/07_akvamarin.jpg" alt="" />
                </a>
                <a
                  href="https://www.i-project.by/portfolio_newsite/akvamarin/14_akvamarin.webp"
                  data-fancybox="gallery"
                >
                  <StaticImage src="../../../images/portfolio/akvamarin/14_akvamarin.jpg" alt="" />
                </a>
                <a
                  href="https://www.i-project.by/portfolio_newsite/akvamarin/09_akvamarin.webp"
                  data-fancybox="gallery"
                >
                  <StaticImage src="../../../images/portfolio/akvamarin/09_akvamarin.jpg" alt="" />
                </a>
                <div class="content_task">
                  <span class="content_task_span">Описание</span>
                  <p class="content_task_p">Квартира со&nbsp;свободной планировкой и&nbsp;шикарными видами из&nbsp;окон. Особенность всех зданий жилого комплекса&nbsp;&mdash; отсутствие двух параллельных стен. Все они либо закругляются, либо расходятся лучами от&nbsp;центра. Такое архитектурное решение представляет для дизайнеров своеобразный квест.</p>
                </div>
              </div>
              <div class="content_images">
                <h2 class="content_h2" id="anchor_2">Планировка квартиры</h2>
                <div class="content_images_two">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/17_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/17_akvamarin.jpg" alt="" />
                  </a>
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/18_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/18_akvamarin.jpg" alt="" />
                  </a>
                </div>
              </div>
              <div class="content_text">
                <h2 class="content_h2" id="anchor_3">Гостиная</h2>
                <p class="content_p">Интерьер выполнен в&nbsp;теплых тонах в&nbsp;соответствии с&nbsp;пожеланиями заказчика. Для оформления стен использованы разные виды шпона, краски и&nbsp;обоев. Варианты отделок комбинируются в&nbsp;зависимости от&nbsp;зоны. Так, например, в&nbsp;местах, где стены подвержены загрязнению, используются износостойкие материалы.</p>
              </div>
              <div class="content_images">
                <div class="content_images_one">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/01_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/01_akvamarin.jpg" alt="" />
                  </a>
                </div>
                <div class="content_images_two">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/02_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/02_akvamarin.jpg" alt="" />
                  </a>
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/03_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/03_akvamarin.jpg" alt="" />
                  </a>
                </div>
                <div class="content_images_one">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/05_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/05_akvamarin.jpg" alt="" />
                  </a>
                </div>
                <div class="content_images_two">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/04_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/04_akvamarin.jpg" alt="" />
                  </a>
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/06_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/06_akvamarin.jpg" alt="" />
                  </a>
                </div>
              </div>
              <div class="content_text">
                <h2 class="content_h2" id="anchor_4">Кухня</h2>
                <p class="content_p">Кухня совмещена с&nbsp;гостиной и&nbsp;столовой. Здесь уместились барная стойка, обеденный стол и&nbsp;угловой диван с&nbsp;ТВ зоной.</p>
              </div>
              <div class="content_images">
                <div class="content_images_one">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/07_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/07_akvamarin.jpg" alt="" />
                  </a>
                </div>
              </div>
              <div class="content_text">
                <h2 class="content_h2" id="anchor_5">Прихожая</h2>
                <p class="content_p">В&nbsp;прихожей находится вместительная гардеробная. Входная зона отделяется от&nbsp;кухни с&nbsp;помощью раздвижной перегородки.</p>
              </div>
              <div class="content_images">
                <div class="content_images_one">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/08_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/08_akvamarin.jpg" alt="" />
                  </a>
                </div>
              </div>
              <div class="content_text">
                <h2 class="content_h2" id="anchor_6">Спальня</h2>
                <p class="content_p">Большой балкон присоединили к&nbsp;квартире и&nbsp;разделили на&nbsp;две части. Это позволило увеличить площадь хозяйской спальни и&nbsp;одной из&nbsp;детских.</p>
              </div>
              <div class="content_images">
                <div class="content_images_one">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/09_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/09_akvamarin.jpg" alt="" />
                  </a>
                </div>
                <div class="content_images_two">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/10_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/10_akvamarin.jpg" alt="" />
                  </a>
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/11_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/11_akvamarin.jpg" alt="" />
                  </a>
                </div>
              </div>
              <div class="content_text">
                <h2 class="content_h2" id="anchor_7">Детские комнаты</h2>
                <p class="content_p">Детские комнаты находятся в&nbsp;общей спальной зоне, отделенной от&nbsp;гостиной коридором. Для этой зоны оборудован вместительный санузел и&nbsp;постирочная, а&nbsp;при гостиной находится второй санузел.</p>
              </div>
              <div class="content_images">
                <div class="content_images_two">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/12_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/12_akvamarin.jpg" alt="" />
                  </a>
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/13_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/13_akvamarin.jpg" alt="" />
                  </a>
                </div>
                <div class="content_images_one">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/14_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/14_akvamarin.jpg" alt="" />
                  </a>
                </div>
                <div class="content_images_two">
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/15_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/15_akvamarin.jpg" alt="" />
                  </a>
                  <a
                    href="https://www.i-project.by/portfolio_newsite/akvamarin/16_akvamarin.webp"
                    data-fancybox="gallery"
                  >
                    <StaticImage src="../../../images/portfolio/akvamarin/16_akvamarin.jpg" alt="" />
                  </a>
                </div>
              </div>
            </div>     
          </div>
        </div>
      </Fancybox>
    </Layout>
  )
};

export default akvamarin;
